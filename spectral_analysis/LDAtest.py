...
def newDatasetWithAdditiveNoise(dataset, fraction):
...
def createDatasetFromBasisSet(N, basisSet):
    ...
    # print(np.shape(basisSet))
    # print(basisSet)
    data = []
    labellist = []
    for i in range(N):
        weight = random.uniform(0.7, 1)
        base = random.randint(0, m-1)
        labellist.append(base)
        data.append(basisSet[base] * weight)
...
def test14MeanSpectrumCalculation(self):
    ...
    N = 100
    basisSet = self.createBasisSet(self.X, N=5, maxPeaks=5, maxAmplitude=1, maxWidth=30, minWidth=5)
    dataset, labels = self.createDatasetFromBasisSet(N=N, basisSet=basisSet)
...
def test15FitPCAWithMoreComplexBasisSet(self):
    ...
    pca = PCA(n_components=2)
    pca.fit(noisyDataset, labels)
    fig, ax = plt.subplots()
    plt.plot(pca.coef_.transpose())
    ax.set_title("Keeping only 2 components")
    ...
    fig, ax = plt.subplots()
    plt.plot(pca.coef_.transpose())
    ax.set_title("Keeping only 4 components")
...
def test16ExpressOriginalBasisVectorInNewObtainedEigenvectorBase(self):
    ...
    recoveredBasisSetWrong = basisCoefficients @ pca.coef_
...
def lda():
    ...
    N = 100000
    basisDimension = 5
    basisSet = createBasisSet(np.linspace(0,500,501), N=basisDimension, maxPeaks=3, maxAmplitude=1, maxWidth=30, minWidth=5)
    dataset, labels = createDatasetFromBasisSet(N=N, basisSet=basisSet)
    dataset = newDatasetWithAdditiveNoise(dataset, fraction=0.5)
    ...
    pca = PCA(n_components=componentsToKeep)
    pca.fit(dataset, labels)
    LDADATA = pca.transform(dataset)

    fig = px.scatter(LDADATA, x=0, y=1, color=labels)
    plot(fig)

    fig = px.scatter(LDADATA, x=2, y=3, color=labels)
    plot(fig)
    ...
    plt.plot(abs(pca.coef_.T))
    plt.title('LDA coefs')
    plt.legend(['0', '1', '2', '3', '4'])
    # plt.yscale('log')
    plt.show()

    plt.plot(abs(pca.scalings_))
    plt.title('LDA scalings')
    plt.legend(['0', '1', '2', '3', '4'])
    # plt.yscale('log')
    plt.show()

    plt.plot(np.array(dataset).T)
    plt.title('raw data')
    plt.show()
...
def createNormalizedNoisyDataset(self, nSamples=100, basisDimension=5, maxPeaks=3, maxAmplitude=1, maxWidth=30,
                                 minWidth=5, noiseFraction=0.1):
